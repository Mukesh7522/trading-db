...
const Sidebar = ({ open, onToggle, selectedStock, onStockSelect }) => {
  ...
  const loadStocks = async () => {
    try {
      ...
      console.log('===========================================');
      console.log('📊 LOADING STOCKS - SIDEBAR');
      console.log('Stocks loaded:', data.length);
      console.log('First stock:', data[0]);
      console.log('Updated date field:', data[0]?.updated_date);
      console.log('===========================================');
      
      // Get the last updated date from the first stock
      if (data && data.length > 0 && data[0] && data[0].updated_date) {
        const dbTimestamp = data[0].updated_date;
        ...
        if (!isNaN(date.getTime())) {
          ...
          console.log(`IST Time: ${istHours}:${istMinutes}`);
          
          // Format
          const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
          const hours12 = istHours === 0 ? 12 : istHours > 12 ? istHours - 12 : istHours;
          const ampm = istHours >= 12 ? 'PM' : 'AM';
          const minutesStr = istMinutes.toString().padStart(2, '0');
          
          const finalText = `${months[utcMonth]} ${istDate}, ${utcYear} at ${hours12}:${minutesStr} ${ampm} IST`;
          
          console.log('FINAL TEXT:', finalText);
          console.log('Setting lastUpdated state to:', finalText);
        ...
        console.warn('⚠️ No updated_date found');
        console.log('Data structure:', { hasData: !!data, length: data?.length, firstStock: data?.[0] });
      }

      calculateNextUpdate();
    } catch (err) {
      console.error('❌ Error in loadStocks:', err);
  ...
  const calculateNextUpdate = () => {
    ...
    // Set next update time to 9:00 AM IST
    let nextUpdateTime = new Date(istTime);
    nextUpdateTime.setHours(9, 0, 0, 0);
    
    // If current IST time is past 9 AM, set to next day
    if (istTime.getHours() >= 9) {
      nextUpdateTime.setDate(nextUpdateTime.getDate() + 1);
    }
    
    // Format next update date
    ...
    setNextUpdate(`${nextUpdateFormatted} at 9:00 AM IST`);
    
    // Calculate time remaining
    const timeRemaining = nextUpdateTime - istTime;
